"""Inventory XSIAM parsing rules from the Cortex content repo."""
...
DEFAULT_OUTPUT_DIR = Path("examples/xql_parsing")
RULE_EXT = ".xif"
YAML_EXT = ".yml"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f"Directory for merged parsing rules (default: {DEFAULT_OUTPUT_DIR}).",
    )
...
def iter_parsing_folders(repo, pack_path: str) -> Iterable[str]:
    """Yield ParsingRules directories inside a pack."""

    parsing_path = f"{pack_path}/ParsingRules"
    try:
        entries = repo.get_contents(parsing_path)
    except Exception as exc:
        logger.debug("Pack %s missing ParsingRules (%s)", pack_path, exc)
        return []
...
def main(argv: list[str] | None = None) -> int:
    ...
    for pack in repo.get_contents("Packs"):
        ...
        folders = list(iter_parsing_folders(repo, pack.path))
        if not folders:
            continue

        logger.info("Found %s parsing folders in %s.", len(folders), pack.path)
        for folder in folders:
            ...
            if not (rules_file and yaml_file):
            ...
            try:
                ...
                logger.warning("Failed to fetch parsing components for %s: %s", folder.path, exc)
                continue
    ...
    logger.info("Completed. Stored %s parsing rules in %s.", merged, args.output_dir)
    return 0

"""Inventory Cortex integrations from the content repo."""
...
DEFAULT_OUTPUT_DIR = Path("examples/integrations")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f"Directory for downloaded integrations (default: {DEFAULT_OUTPUT_DIR}).",
    )
...
def iter_integration_files(repo, integration_path: str) -> list:
    entries = repo.get_contents(integration_path)
    files = []
    for entry in entries:
        if entry.type == "file" and entry.path.endswith(SUPPORTED_SUFFIXES):
            files.append(entry)
        elif entry.type == "dir":
            files.extend(iter_integration_files(repo, entry.path))
    return files
...
def main(argv: list[str] | None = None) -> int:
    ...
    for pack in repo.get_contents("Packs"):
        if pack.type != "dir":
            continue
        integrations_dir = f"{pack.path}/Integrations"
        try:
            repo.get_contents(integrations_dir, ref=repo.default_branch)
        except Exception:
            continue

        files = iter_integration_files(repo, integrations_dir)
        if not files:
            continue

        logger.info("Found %s integrations in %s.", len(files), integrations_dir)
    ...
    logger.info("Completed. Downloaded %s integration files into %s.", downloaded, args.output_dir)
    return 0
